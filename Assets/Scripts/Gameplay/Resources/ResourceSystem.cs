...
public class ResourceSystem : MonoBehaviour
{
    ...
    // Make sure the resource dictionary exists before any access
    private void EnsureDictionaryExists()
    {
        if (resourceDictionary == null)
        {
            Debug.LogWarning("ResourceSystem: resourceDictionary was null. Reinitializing.");
            InitializeResources();
        }
    }
    ...
    public bool SpendResources(List<ResourceAmount> costs)
    {
        ...
        foreach (ResourceAmount cost in costs)
        {
            if (!resourceDictionary.ContainsKey(cost.resourceType))
            {
                resourceDictionary[cost.resourceType] = 0;
            }
